...
Живая карта мира, на которой в реальном времени загораются места, чьи статьи в Википедии правят прямо сейчас. Плюс дашборд с аналитикой поверх истории правок. Данные для
...
Два экрана: живая карта, где правки за последние 30 минут сгруппированы
в H3-ячейки, и дашборд исторических витрин. React, TypeScript, Vite,
JS API Яндекс Карт v3 и `h3-js`.

Подробнее —
в [`docs/architecture/frontend.md`](docs/architecture/frontend.md).
...
Первый способ быстрого старта работает без кластера, но для полной работы
нужен ключ Яндекс Карт. Без него тоже запустится, дашборд заработает
целиком, но карта вместо тайлов покажет экран ошибки.
