...
public class H3GeoService {
     ...
     * ячейка. Границы задаются ключами {@code app.live.zoom-r*-max} и подобраны
     * по плотности гексагонов на экране.
     *
     * <p>Резолюция 7 в лестнице пропущена: ступеней шесть на тридцать значений
     * зума, и семёрка в эту раскладку не попала.
     */
    ...
    /**
     * Задевает ли ячейка видимый прямоугольник. Проверка идёт с двух сторон,
     * и обе нужны: {@code hexagonTouchesBox} ловит обычный случай, когда ячейка
     * мельче экрана, а {@code boxTouchesHexagon} — сильно отдалённую карту,
     * где экран целиком помещается внутрь одной ячейки и ни центр, ни вершины
     * в него не попадают.
     *
     * <p>Честного пересечения многоугольников здесь нет: ячейка, задевающая
     * экран только краем, в ответ не попадёт. Промах возможен лишь по периметру
     * видимой области и стоит дешевле пересечения полигонов на каждом опросе.
     */
