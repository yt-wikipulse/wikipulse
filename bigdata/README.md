...
команды проверки ниже используют именно его.
...
В примерах ниже `YT_PROXY` подставляется в `--master` целиком, поэтому
задавайте его со схемой: `export YT_PROXY=https://<proxy-host>`. В путях
Cypress используется `yt:/$YT_BASE_PATH/...`: одиночный слэш после `yt:`
плюс путь, начинающийся с `//`, даёт ровно `yt:///...`, как ждёт SPYT.
...
Шаги 1–3 выполняются один раз на кластер, шаги 4–8 работают постоянно.
`deploy/compose.yml` поднимает в контейнерах шаги 4 и 8; шаги 1–3 всё равно
делаются руками, до первого запуска compose.
...
со `spark.pyspark.python` в командах запуска ниже. Для другого образа кластера
правьте обе константы вместе.
...
Работает 24/7. В заголовках запросов уходит `WIKIPULSE_CONTACT` — задайте
его, если ингестор работает долго.
...
идёт следом за архиватором. Обычно архиватор запускает шедулер (шаг 8) —
руками он нужен только для разового прогона.
...
Пока шедулер работает, не запускайте `archiver` руками: два архиватора идут
по одному курсору и дублируют строки в `t_history`. В `deploy/compose.yml`
отдельного архиватора нет по этой же причине — там только `ingestor` и
`scheduler`.
