#!/usr/bin/env python3
"""
Загрузка справочника координат ``dict/coords`` из дампа Wikidata.

Дамп (десятки гигабайт) не скачивается на диск: он читается одним
потоковым соединением и разжимается на лету, строки пишутся во временную
таблицу пачками. Дешёвая проверка на подстроку ``"P625"`` отсеивает
подавляющее большинство сущностей до разбора JSON.

Итоговая таблица строится в три шага — сортировка, ``run_reduce``
с дедупликацией по ``(wiki, title)`` в заранее созданную таблицу
с ``unique_keys = true``, затем ``alter_table(dynamic=True)`` и
монтирование. Включить ``unique_keys`` у существующей таблицы нельзя,
а без него не работает ``lookup_rows``, которым справочник читает
обогащение. Дедупликация обязательна: в дампе встречаются разные
сущности с одинаковой парой ``(wiki, title)``.
"""
import json
import gzip
import time
...
def unique_schema():
    """Схема справочника: сортировка по ``(wiki, title)`` и ``unique_keys``."""
