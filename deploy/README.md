...
| `YMAPS_API_KEY` | ключ JavaScript API Яндекс Карт, бэкенд отдаёт его фронту |
...
`YMAPS_API_KEY`, иначе карта не загрузится. Ключ живёт в окружении бэкенда
и уходит фронту по `GET /api/v1/config`, поэтому его смена требует только
`docker compose up -d backend`, без пересборки `web`.
