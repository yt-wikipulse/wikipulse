...
`script-src` строгий, поэтому инлайновый скрипт в `frontend/index.html`
(переключение фавиконки под тему системы) блокируется и пишет нарушение
в консоль; фавиконка остаётся светлой, на остальное это не влияет.
