...
хост, без схемы и без `/*`. Referer нужно прописать **каждому** ключу из
`VITE_YMAPS_API_KEY`: фронт берёт из списка случайный при каждой загрузке,
и забытый ключ ломает карту примерно через раз. Ключ подставляется в
