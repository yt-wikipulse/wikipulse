...
Итоговое ТЗ также требует дашборд с исторической аналитикой. Он реализован:
маршрут `/dashboard` показывает данные из `GET /api/v1/dashboard` и не должен
показывать ничего, кроме них.
...
между B и F1 закреплена в контракте `GET /api/v1/dashboard`
(`../03-contracts/rest-api.md`): периоды, шаг графика, поведение на пустых
данных и на ошибках описаны там.
