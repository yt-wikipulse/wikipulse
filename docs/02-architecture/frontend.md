...
Второй экран — дашборд с исторической аналитикой: маршрут `/dashboard`
показывает данные из `GET /api/v1/dashboard` и не должен показывать ничего,
кроме них.
...
| Каким правилам подчиняется код | [Frontend conventions](frontend-conventions.md) |
...
| Как вести ветки, review и документацию | [CONTRIBUTING](../../CONTRIBUTING.md) |
...
adapter или внешняя схема расходятся, стык сначала согласуется и контракт
правится, а расхождение не маскируется в UI.
...
Маршрут `/dashboard` показывает историческую аналитику из витрин. Граница
с бэкендом закреплена в контракте `GET /api/v1/dashboard`
([../03-contracts/rest-api.md](../03-contracts/rest-api.md)): периоды, шаг
графика, поведение на пустых данных и на ошибках описаны там.

Данные берутся только из этого эндпоинта. Нельзя добавлять frontend mocks,
рисовать графики по выдуманным числам или вычислять историческую аналитику из
данных живого окна карты — окно и витрины имеют разную свежесть и разный
смысл.
...
Обязательные правила SCSS Modules, BEM, tokens, интерактивных состояний и
адаптивности находятся в [frontend-styles.md](frontend-styles.md).

## Проверка изменений

Команды frontend перечислены в [`frontend/README.md`](../../frontend/README.md).
Что запускать перед review — в [CONTRIBUTING.md](../../CONTRIBUTING.md).
Актуальный набор проверок выбирается по затронутому коду; непроведённая browser
или production-проверка не объявляется выполненной.
