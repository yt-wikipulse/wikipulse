...
`.env.example`. Переменная читается в `loadYmaps` и попадает в бандл на
сборке, поэтому после правки `.env` dev server надо перезапустить.

### Карта не грузится ближе к вечеру, хотя ключ задан

У бесплатного ключа JavaScript API лимит 100 загрузок карты в сутки. В
`VITE_YMAPS_API_KEY` можно перечислить несколько ключей через запятую —
`loadYmaps` выбирает случайный при каждой загрузке страницы, и суточный
лимит умножается на их число. Если исчерпаны все ключи, помогает только
новый ключ или следующие сутки.
