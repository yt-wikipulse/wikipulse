...
**Карта на запрос в YTsaurus не обращается**: данные уже лежат в памяти,
их положил туда фоновый поллер, и запрос стоит одной свёртки по кэшу.
Дашборд читает витрины из кластера, но через кэш ответа на 60 секунд,
поэтому в кластер уходит не более одного чтения в минуту на пару
`(period, limit)`. Данные разной природы: окно в 30 минут помещается
в память, агрегаты за месяц на лету не считаются, но и меняются раз в час.
...
Первый тик после старта поднимает позицию из консьюмера `c_backend`
(`committedOffsets`) и число партиций очереди. Дальше каждый тик обходит
все партиции и читает страницами (до 10 страниц по 1000 строк) через
`QEnrichedRepository.pull`, пока в очереди есть непрочитанное.

Консьюмер двигается в `finally` — после страницы, независимо от того, что
...
На ошибке чтения (`YtReadException`) позиция не двигается вовсе, и следующий
тик перечитает ту же порцию.

Позиция живёт в кластере, а не в памяти, поэтому рестарт продолжает чтение
с того места, где оно прервалось. На пустом консьюмере чтение начинается
с начала очереди: догон идёт порциями по тику, а всё, что старше окна,
кэш выбрасывает сам.
...
Событие отбрасывается при укладке в кэш, с сообщением в лог, в двух случаях:
...
Наружу отдаются читаемые периоды `24h`, `7d` и `30d`, а витрины хранят
период строкой в часах, поэтому сервис переводит одно в другое: `7d` →
`168h`, `30d` → `720h`.
...
Spring MVC, из-за чего 404 и 405 отдаются как 500.
...
`MockPoller` сдвигает выборку к текущему времени и прокручивает её по кругу,
`MockDashboardService` читает фикстуры при старте и обрезает топы под
запрошенный `limit`.

**Ячейки в mock настоящие.** В фикстуре правок лежит поле `h3_r9`, снятое
тем же путём, что и на кластере: 382 правки в 288 разных ячейках. `MockPoller`
отдаёт его как есть, поэтому карта на профиле `mock` показывает правки там,
где они действительно были. Синтетическое в фикстуре только время — оно
сдвигается к текущему окну, чтобы события выглядели свежими.

## Известные ограничения

1. **Одна реплика.** Кэш живой карты живёт в памяти процесса, а консьюмер
   `c_backend` рассчитан на одного читателя: вторая реплика будет держать
   свою копию окна и получит от `advance_consumer` конфликт оффсетов.
2. **Свёртка идёт по всему снимку на каждый запрос.** Кэшируется окно,
   но не результат свёртки: два пользователя с одинаковым зумом и экраном
   оплачивают её дважды.
3. **Экран через 180-й меридиан не поддерживается.** Валидация требует
   `min_lng < max_lng`, так что viewport через антимеридиан получит 400.
4. **У каждого репозитория свой `YTsaurusClient`.** Два подключения к одному
   кластеру там, где хватило бы одного бина.
5. **Ключа `app.enrich.fetch-batch` в `application.yaml` нет** —
   `QEnrichedRepository` работает на значении `1000`, заданном по умолчанию
   в `@Value`.
