...
├── features/     # оркестрация сценария: поллинг, отмена, состояния загрузки
...
- **`pages/`** — связывают маршрут, данные и крупные части интерфейса.
  ...
  страница не знает;
- **`features/`** — оркестрация сценария: поллинг, отмена запроса, состояния
  загрузки и ошибки;
- **`components/`** — получают данные и колбэки через props, HTTP не
  делают. Исключение — `LiveMap`: это адаптер императивного API Яндекс Карт,
  он владеет экземпляром карты, объектами-фичами и их освобождением;
...
Оркестрация живой карты (`features/live-map/useLiveMapData.ts`) отвечает за:

- получение viewport и debounce частых перемещений карты;
- поллинг раз в 2,5 секунды без наложения запросов;
- отмену запроса при смене viewport и при unmount;
- защиту от устаревшего ответа, пришедшего после более свежего;
- состояния `loading`, `refreshing`, `error`, `empty`, `success`;
- сохранение последних хороших данных при временной ошибке обновления.
...
Под `StrictMode` эффекты корректно освобождают ресурсы при повторном
монтировании.

## Ближайшая правка

Кнопка на карте открывает панель `NearestEditPanel`. Сценарий
(`features/nearest-edit/`) запрашивает геопозицию у браузера, затем строит
вокруг неё bbox и обходит три радиуса подряд — 55, 220 и 880 км — пока
`GET /api/v1/hexagons/active` не вернёт хотя бы одну ячейку; из полученных
событий выбирается ближайшее к пользователю. Отдельного эндпоинта у сценария
нет, он работает поверх того же ответа, что и карта.

Отказ в доступе и невозможность определить позицию — разные состояния панели.
При отказе она просит разрешить геолокацию в настройках сайта; при таймауте
и неудачном определении позиции предлагает кнопку «Повторить». Если браузер
геолокацию не поддерживает или страница открыта не по HTTPS, повторять нечего,
и кнопки нет.
