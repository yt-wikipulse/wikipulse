...
получает адрес как есть, а `bigdata/src/bigdata/runtime.py` дописывает
`https://`, если схемы нет. На обычном кластере разница незаметна, на
локальном по HTTP — ломает `bigdata`, поэтому там нужен явный `http://`.
