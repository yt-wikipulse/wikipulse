...
адрес в ответе должен резолвиться с вашей машины (`--docker-hostname`).
...
**Схему в `YT_PROXY` здесь пишут явно.** Если её не написать, и бэкенд
(`repository/YtProxy.java`), и `bigdata` (`runtime.proxy_url`) подставят
`https://` — клиент пойдёт на 443, и до локального кластера по HTTP
не доберётся.
...
витрины. Queue agent `run_local_cluster.sh` поднимает самостоятельно.
...
по 100 строк; за минуту набирается пара сотен. Боты и служебные страницы
отсеиваются, поэтому `in` в логе на порядок больше `out`. Остановка — Ctrl+C.
...
прочитает строку и в том случае, если она легла в очередь до запуска
бэкенда.
