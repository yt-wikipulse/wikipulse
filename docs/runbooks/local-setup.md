...
| Ключ Яндекс Карт (JavaScript API v3) | — | без него карта не отрисуется |
...
Dev-сервер проксирует `/api` на `http://localhost:8080`, поэтому бэкенд должен
...
`backend/src/main/resources/fixtures/`: живая карта прокручивает по кругу
снимок правок Википедии, дашборд отдаёт три снятые витрины. Координаты
в фикстурах тоже настоящие, поэтому правки стоят на своих местах.
Синтетическое здесь только время: живого потока нет, времена сдвигаются
к текущему окну, а числа дашборда не меняются. Подробнее — в
