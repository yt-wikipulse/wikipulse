...
правок Википедии, дашборд отдаёт три снятые витрины. Настоящие в фикстурах
и координаты, поэтому правки стоят на своих местах. Синтетическое здесь
только время: живого потока нет, времена сдвигаются к текущему окну, а числа
дашборда не меняются. Подробнее — в [backend.md](../architecture/backend.md).
