...
2. Ключ Яндекс.Карт фронтенду не нужен: он приходит с бэкенда по
   `GET /api/v1/config`. Задаётся он в окружении бэкенда переменной
   `YMAPS_API_KEY`, а dev server проксирует `/api` на `localhost:8080`,
   поэтому для карты нужен запущенный бэкенд.
