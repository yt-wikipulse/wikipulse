...
2. Создать `.env` на основе `.env.example` и указать ключи Яндекс.Карт через
   запятую. У бесплатного ключа лимит 100 загрузок карты в сутки, поэтому
   при каждой загрузке страницы берётся случайный ключ из списка:

   ```dotenv
   VITE_YMAPS_API_KEY=<KEY_1>,<KEY_2>,<KEY_3>,<KEY_4>,<KEY_5>
   ```
