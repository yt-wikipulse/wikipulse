...
/**
 * Строка состояния карты: число ячеек в кадре и окно, за которое они собраны.
 *
 * Ноль ячеек показывается как «Нет правок»: «0 ячеек» читается как поломка,
 * хотя пустой вьюпорт — нормальное состояние.
 *
 * Фоновая ошибка не заменяет счётчик: пока данные есть, остаётся число
 * и добавляется значок предупреждения, а кнопка «Повторить» появляется только
 * когда показывать нечего. Кнопка исчезает сама, когда фоновый опрос всё-таки
 * прошёл, поэтому фокус с неё возвращается на корень блока — иначе он упал бы
 * в `body` посреди работы с клавиатуры.
 */
