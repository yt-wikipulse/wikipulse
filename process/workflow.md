...
ждут друг друга. Контракты живут в [`docs/03-contracts/`](../docs/03-contracts/),
...
| Решения пишем строкой в [`docs/04-decisions/`](../docs/04-decisions/) | на защите спросят «почему так» |
...
- если менялся контракт — обновлён [`docs/03-contracts/`](../docs/03-contracts/)
  и затронутые уведомлены.

**У задачи без кода — тот же порядок.** Исследование, схема, спайк, выбор
источника данных заканчиваются PR в `docs/`: документом, схемой или строкой
в [ADR](../docs/04-decisions/). Работа, которая существует только в переписке
или в чьей-то голове, не сделана: её нельзя ни переиспользовать, ни
отревьюить, и она теряется, если человек выпал.

Это не отчёт перед кем-то. Это условие, при котором результат остаётся
у команды, а не у автора.
